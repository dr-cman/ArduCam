...
int ArduCAM::wrSensorRegs8_8(const struct sensor_reg reglist[])
{
	  ...
	  while ((reg_addr != 0xff) | (reg_val != 0xff))
	  {
	    ...
	    //err = wrSensorReg8_8(reg_addr, reg_val);  // warning: variable 'err' set but not used
...
int ArduCAM::wrSensorRegs8_16(const struct sensor_reg reglist[])
{
	  ...
	  while ((reg_addr != 0xff) | (reg_val != 0xffff))
	  {
       ...
       reg_val = next->val;
	    ...
	    // err = wrSensorReg8_16(reg_addr, reg_val); // warning: variable 'err' set but not used
	    //  if (!err)
	    //return err;
	    next++;
		#if defined(ESP8266)
...
int ArduCAM::wrSensorRegs16_8(const struct sensor_reg reglist[])
{
	  ...
	  unsigned char reg_val=0x00;  // warning: 'reg_val' may be used uninitialized in this function
	  const struct sensor_reg *next = reglist;
